...
defmodule LowendinsightGet.Analysis do
  ...
  def process(uuid, urls, start_time) do
    ...
    repos = urls
      |> Task.async_stream(__MODULE__, :analyze, ["lei-get"], [timeout: :infinity, max_concurrency: 10])
      |> Enum.map(fn {:ok, report} -> elem(report, 1) end)

    report = %{state: "complete", report: %{uuid: UUID.uuid1(), repos: repos}, metadata: %{repo_count: length(repos)}}
